...
class font_cache : public l2list { 
  ...
  public: 
	...
	wchar_t wtext[20];
	mbstowcs(wtext, font_name[type], strlen(font_name[type]) + 1);//Plus null
...
{ 
	...
	if (bgiemu_handle_redraw || visual_page != active_page) {
		...
		TextOut(hdc[1], x, y, wtext, strlen(str));
    }
    if (visual_page == active_page) { 
		TextOut(hdc[0], x, y, wtext, strlen(str));
    } 
}
...
int textheight(const char* str)
{
	...
	wchar_t wtext[20];
	mbstowcs(wtext, str, strlen(str) + 1);//Plus null
	GetTextExtentPoint32(hdc[0], wtext, strlen(str), &ss);
    return ss.cy;
}
...
void initgraph(int* device, int* mode, char const* /*pathtodriver*/)
{
    ...
    if (wcApp.lpszClassName == NULL) { 
		...
		char text[] = "BGIlibrary";
		wchar_t wtext[20];
		mbstowcs(wtext, text, strlen(text) + 1);//Plus null
		LPWSTR ptr = wtext;

		wcApp.lpszClassName = ptr;
	wcApp.hInstance = 0;
	...
	char text[] = "Windows BGI";
	wchar_t wtext[20];
	mbstowcs(wtext, text, strlen(text) + 1);//Plus null
	LPWSTR ptr = wtext;

	char text1[] = "BGIlibrary";
	wchar_t wtext1[20];
	mbstowcs(wtext1, text1, strlen(text1) + 1);//Plus null
	LPWSTR ptr1 = wtext1;

	hWnd = CreateWindow(ptr1, ptr,
			WS_OVERLAPPEDWINDOW,
